...
export default function Home() {
  const { append, messages, input, handleInputChange, handleSubmit } = useChat({
    api: "http://127.0.0.1:6969/conversation"
  });;
  ...
  const [images,setImages]=useState<Array<string>>(['figure-1-1.jpg']);
  ...
  const handleSendImpg = async () => {

    try {
      const response = await fetch('http://127.0.0.1:6969/impg', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        const updateStateFromData = (data) => {
          // Set 'images' from 'data.images'
          setImages(data.images);
          if(data.page_number.length<=0){
            // Extract page numbers from image filenames
            const extractedPageNumbers = data.images.map(image => {
              const matches = image.match(/figure-(\d+)-\d+\.jpg/i);
              return matches ? parseInt(matches[1]) : null;
            }).filter(pageNumber => pageNumber !== null);
            
            // If any page numbers are found, set 'pageNumbers' state
            if (extractedPageNumbers.length > 0) {
              setPageNumbers(Array.from(new Set(extractedPageNumbers))); // Remove duplicates
            }
          }else{
            setPageNumbers(data.page_number)
          }
      };
  ...
  return (
    ...
    <main className="flex h-screen flex-row items-center justify-center">
      <section className='chatbot-section flex flex-col origin:w-[1200px] w-full origin:h-[735px] h-full rounded-md p-2 md:p-6'>
        <div className='chatbot-header pb-6'>
          <div className='flex justify-between'>
            <div className='flex items-center gap-2'>
              ...
              <h1 className='chatbot-text-primary text-xl md:text-2xl font-medium'>Doc Bot</h1>
            </div>
          ...
          <p className="chatbot-text-secondary-inverse text-sm md:text-base mt-2 md:mt-4">Chat with your Documents</p>
        </div>
      ...
      <section className='chatbot-section flex flex-col origin:w-[800px] w-full origin:h-[700px] h-full rounded-md p-2 md:p-6 border border-gray-300'>

        <div className='pdf-container' style={{ height: '60%', overflowY: 'auto' }}>
          <h1 className='text-white'>Relevant PDF Pages</h1>
          <Document file="file_merged.pdf">
            {pageNumbers.map((pageNumber, index) => (
              <Page key={index} pageNumber={pageNumber as number} renderAnnotationLayer={false} renderTextLayer={false} />
            ))}
          </Document>
        </div>
        <h1 className='text-white'>Relevant Images</h1>
        <div className='flex flex-row p-1' style={{ height: '40%' }}>
          {/* <div className='image-container p-1' style={{ width: '50%' }}>
            <img src="image.png" alt='Random Image' style={{ width: '100%', height: '100%' }} />
          </div> */}
          {images.map((image, index) => (
              <div key={index} className='image-container p-1' style={{ width: '50%' }}>
                <img src={"figures/" + image} alt='Random Image' style={{ width: '100%', height: '100%' }} />
              </div>
            ))}
